...
if track
    ...
    siz=size(WP);
    for ii=1:(siz(2)-1)
        plot([WP(2,ii), WP(2,ii+1)], [WP(1,ii), WP(1,ii+1)], 'r-x', 'LineWidth', 1)
    end
end
wp_plot = plot(y, x,'--','LineWidth', 1)
L=300;
...
for now=1:dec:length(x)
    ...
    boat = tmpR*2*[L/2 .9*L/2 .5*L/2 -L/2 -L/2 .5*L/2 .9*L/2 L/2; 
              0 10 20 20 -20 -20 -10 0];
    boatpath = plot(y(now)+boat(2,:),x(now)+boat(1,:),'r');
    shipplot = patch(y(now)+boat(2,:),x(now)+boat(1,:),'y');
    
    tnow=tnow+tsamp*dec;
end
hold off
legend([wp_plot boatpath shipplot], 'boat path', 'waypoint path', 'boat');
xlabel('East [m]')
ylabel('North [m]')
